import { loadResources, type Resource, getImageUrl } from "@/loaders/loadResources";
...
function ResourceCard({ resource }: ResourceCardProps) {
  const imageUrl = resource.image ? getImageUrl(resource.image) : "";
    ...
    const icons = {
      guide: (
        <svg className="w-6 h-6 text-[#e25002]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
        </svg>
      ),
      community: (
      ...
      form: (
        <svg className="w-6 h-6 text-[#fdb52b]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
        </svg>
      ),
      video: (
        <svg className="w-6 h-6 text-[#e25002]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {videos.length > 0 && (
        <section className="py-12 sm:py-16 px-4 sm:px-6 lg:px-8 bg-gray-50">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {videos.map((resource) => (
                <div key={resource._id} className="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow">
                  <div className="bg-gray-200 h-48 sm:h-64 lg:h-72">
                    <img 
                      src={imageUrl}
                      alt={resource.image.alt || resource.title}
                      className="w-full h-full object-cover"
                  ...
                  <div className="p-4">
                    <h3 className="text-lg font-bold text-[#485a61] mb-2">{resource.title}</h3>
                    <p className="text-[#485a61] opacity-80 text-sm mb-3">{resource.description}</p>
                    <span className="text-xs text-[#19b8d7]">{resource.duration}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="text-center mt-8">
              <button className="bg-[#19b8d7] hover:bg-[#1596af] text-white px-6 lg:px-8 py-3 lg:py-4 rounded-lg text-base lg:text-lg font-semibold transition-colors">
                View All Videos
              </button>
            </div>
          </div>

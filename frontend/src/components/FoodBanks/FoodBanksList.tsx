			...
			{foodBanks.map((foodBank) => (
				<div key={foodBank._id} className="bg-white p-6 rounded-lg shadow-lg border border-gray-200">
					<div className="flex flex-col items-start gap-4">
						<h3 className="text-xl font-bold text-(--dark-blue) ">{foodBank.name}</h3>
						<div className="space-y-2 text-sm text-(--dark-blue)">
							{foodBank.link && (
								<p className="text-blue-600 text-md font-bold"><a href={foodBank.link}>External Link</a></p>
							)}
							{foodBank.phone && (
								<p>
							...
							<p className="font-bold">{ foodBank.appointmentOnly ? <p>Appointment Only</p> : "" }</p>
						</div>
					</div>
				</div>
			))}
